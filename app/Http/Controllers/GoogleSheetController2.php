...
public function updateGoogleSheet()
{
    ...
    $all_routes_grouped = Routes::where('time', '=', 'Evening')
    ...
    $summaries = DB::table('orders')
        ...
        ->select('carts.item_number', 'shops.evening_route', DB::raw('SUM(carts.qty) as total_qty'))
        ->groupBy('carts.item_number', 'shops.evening_route')
        ->get()
        ->keyBy(function ($item) {
            return $item->item_number . '_' . $item->evening_route;
        });
