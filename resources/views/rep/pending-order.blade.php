...
@section('title', 'Rep | Pending Orders')
...
<div class="d-flex flex-wrap justify-content-between align-items-end gap-2 mb-4">
    <div>
        ...
        <div class="page-sub">Dashboard / Pending Orders — {{ count($Orders) }} order(s)</div>
    </div>
</div>
...
<div class="panel">
    <div class="panel-head">
        <div>
            ...
            <div class="sub">Your assigned shops pending orders</div>
        </div>
        <form action="/rep/pending-order" method="GET" class="d-flex gap-2">
    ...
    <div class="table-responsive">
        <table class="table table-bakery align-middle">
            ...
            <tbody>
                ...
                @php $fid = 'note-'.$loop->iteration; @endphp
                <tr>
                    <td>{{ $loop->iteration }}</td>
                    <td class="mono">{{ $order->unique_id }}</td>
                    <td>{{ $order->shop_name }}</td>
                    <td><span class="status-pill status-pending">{{ $order->time_period }}</span></td>
                    ...
                    <td style="min-width:200px;">
                        ...
                        <form action="/rep/pending-order-note-update" method="POST" id="{{ $fid }}" class="d-none">
                            @csrf
                        </form>
                    </td>
                    <td class="mono">රු. {{ number_format($order->total_price, 2) }}</td>
                    <td>
                        <a href="{{ route('rep.pending-orders-view', ['id' => $order->unique_id, 'note' => $order->note, 'shop_name' => $order->shop_name]) }}" class="btn btn-soft btn-sm">View</a>
                    </td>
                </tr>
                @empty
