...
@section('title', 'Cart | Perera Bakers')
...
<div class="d-flex flex-wrap justify-content-between align-items-end gap-2 mb-4">
    <div>
        ...
        <div class="page-sub">Dashboard / Create Order / Cart — <span class="mono">{{ session('selected_shop') }}</span></div>
    </div>
    <div>
        <a href="/order-admin/create-order?shop={{ session('selected_shop') }}" class="btn btn-soft">Go Back</a>
    </div>
</div>
        ...
        <table class="table table-bakery align-middle">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><input type="number" name="qty" class="form-control form-control-sm" min="0" step="0.01" required pattern="[0-9]+(\.[0-9]+)?" value="{{ $cart->qty }}" form="{{ $fid }}" style="min-width:100px;"></td>
                    <td><input type="text" class="form-control form-control-sm" name="remarke" value="{{ $cart->remarke }}" form="{{ $fid }}" style="min-width:120px;"></td>
                    <td class="mono">රු. {{ number_format($cart->price * $cart->qty, 2) }}</td>
                    <td>
                        <form action="/order-admin/cart/update-qty" method="POST" id="{{ $fid }}">
                            @csrf
                    ...
                    <td><button type="submit" form="{{ $fid }}" formaction="/order-admin/cart/delete-item" onclick="return confirmDelete();" class="btn btn-soft btn-sm" style="color:var(--accent);">Delete</button></td>
                </tr>
...
<div class="panel">
    <div class="panel-head">
        <div>
            <h2>Place Order</h2>
            <div class="sub">Add a note and confirm the order</div>
        </div>
    </div>
    <form action="/order-admin/cart/order-process" method="POST" class="row g-3">
        @csrf
        <div class="col-12 col-md-6">
            <label for="order_note" class="form-label">Special Note</label>
            <textarea class="form-control" id="order_note" name="note" rows="5"></textarea>
        </div>
        ...
        <div class="col-12 col-md-6 offset-md-3">
            ...
            <button class="btn btn-accent w-100">Proceed</button>
        </div>
    </form>
</div>
