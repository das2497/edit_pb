...
@section('title', 'Pending Orders Add Items | Perera Bakers')

@section('content')

<div class="d-flex flex-wrap justify-content-between align-items-end gap-2 mb-4">
    <div>
        <h1 class="display-font page-title mb-1">Pending Orders — Add Items</h1>
        <div class="page-sub">Dashboard / Pending Orders / <span class="mono">{{ $order_number }}</span> / Add Items</div>
    </div>
    <div>
        <a href="/order-admin/pending-orders-view/{{ $order_number }}" class="btn btn-soft">Go Back</a>
    </div>
</div>
...
<div class="panel">
    ...
    <form action="/order-admin/pending-orders-add-item" method="GET" class="d-flex flex-wrap gap-2 mb-3">
        ...
        <table class="table table-bakery align-middle">
            ...
            <tbody>
                ...
                <tr @if ($inCart) class="in-cart" @endif>
                    ...
                    <td>
                        <form action="{{ route('order-admin-pending-orders-add-items-process') }}" method="POST" id="{{ $fid }}">
                            @csrf
