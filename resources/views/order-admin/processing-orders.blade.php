...
@section('title', 'Processing Orders | Perera Bakers')
...
<div class="d-flex flex-wrap justify-content-between align-items-end gap-2 mb-4">
    <div>
        ...
        <div class="page-sub">Dashboard / Processing Orders</div>
    </div>
</div>
...
<div class="panel">
    <div class="panel-head">
        ...
        <form action="/order-admin/processing-orders" method="GET" class="d-flex gap-2">
        ...
        <table class="table table-bakery align-middle">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><a href="{{ route('order-admin-processing-orders-view', ['id' => $order->unique_id, 'note' => $order->note]) }}" class="btn btn-soft btn-sm">View</a></td>
                </tr>
                @endforeach
            </tbody>
            @else
            <tbody>
                @foreach ($Orders as $order)
                @php $fid = 'pro-' . $loop->iteration; @endphp
                <tr>
                    <td>{{ $loop->iteration }}</td>
                    <td class="mono">{{ $order->unique_id }}</td>
                    <td>{{ $order->name }}</td>
                    ...
                    <td><a href="{{ route('order-admin-processing-orders-view', ['id' => $order->unique_id, 'note' => $order->note]) }}" class="btn btn-soft btn-sm">View</a></td>
                    <td>
                        <form action="/order-admin/processing-orders-update-order" method="POST" id="{{ $fid }}">
                            @csrf
